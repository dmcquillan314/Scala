...
object Test {
  ...
  implicit val session = Database.forURL("jdbc:h2:mem:play", driver = "org.h2.Driver").createSession()
                                                  //> session  : scala.slick.session.Session = scala.slick.session.BaseSession@2f
                                                  //| 79b905
  ...
  }                                               //> res0: Seq[Int] = ArrayBuffer(1, 2, 3, 4, 5)
  ...
  val productsQuery: scala.slick.lifted.Query[models.Test.Products.type, models.Test.Product] =
    for( product <- Products if product.name.startsWith("B")) yield product
                                                  //> productsQuery  : scala.slick.lifted.Query[models.Test.Products.type,models.
                                                  //| Test.Product] = scala.slick.lifted.WrappingQuery@502f1eb9
  ...
  productsQuery.selectStatement: String           //> res1: String = select x2."id", x2."name" from "Products" x2 where x2."name"
                                                  //|  like 'B%' escape '^'
  ...
  productsQuery.list: List[models.Test.Product]   //> res2: List[models.Test.Product] = List(Product(2,Bread), Product(3,Butter))
                                                  //| 
  ...
  val productsWithProperty = session withTransaction {
    ...
    } yield (property, product)                   //> productsWithProperty  : scala.slick.lifted.Query[(models.Test.ProductProper
                                                  //| ties.type, models.Test.Products.type),(models.Test.ProductProperty, models.
                                                  //| Test.Product)] = scala.slick.lifted.WrappingQuery@6998f6dd
  }
 
  // Get generated SQL
  productsWithProperty.selectStatement            //> res3: String = select x2."id", x2."qualifier", x2."value", x2."productId", 
                                                  //| x3."id", x3."name" from "ProductProperties" x2, "Products" x3 where x3."id"
                                                  //|  = x2."productId"
 
  // You could use groupBy to group results by Product
  productsWithProperty.list.groupBy( x => x._2 )  //> res4: scala.collection.immutable.Map[models.Test.Product,List[(models.Test.
                                                  //| ProductProperty, models.Test.Product)]] = Map(Product(1,Milk) -> List((Prod
                                                  //| uctProperty(1,color,white,1),Product(1,Milk))), Product(3,Butter) -> List((
                                                  //| ProductProperty(3,color,yellow,3),Product(3,Butter)), (ProductProperty(4,te
                                                  //| mp,cold,3),Product(3,Butter))), Product(2,Bread) -> List((ProductProperty(2
                                                  //| ,color,brown,2),Product(2,Bread))), Product(4,Sugar) -> List((ProductProper
                                                  //| ty(5,color,white,4),Product(4,Sugar))))
  ...
  val productsByProperty = Parameters[String] flatMap { value =>
    ...
    } yield product                               //> productsByProperty  : scala.slick.driver.BasicQueryTemplate[String,models.T
                                                  //| est.Product] = scala.slick.driver.BasicQueryTemplate@5d612394
  }
  ...
  productsByProperty("white").list                //> res5: List[models.Test.Product] = List(Product(1,Milk), Product(4,Sugar))
  ...
  val productById = Products.byId(2).list         //> productById  : List[models.Test.Product] = List(Product(2,Bread))
